...
function ShowHomePage() {
 ...
 $tables = '';
 $div ="<div style='display: inline-block; vertica;-align: top; padding: 5px'>";
...
WHERE YEAR(reports.date) = YEAR(NOW()) AND MONTH(reports.date) = MONTH(NOW()) AND WEEK(reports.date, 1) = WEEK(NOW(), 1)")[0]['c'];
      ...
      $c_month = SQLToArray("SELECT COUNT(*) AS c FROM reports WHERE YEAR(reports.date) = YEAR(NOW()) AND MONTH(reports.date) = MONTH(NOW())")[0]['c'];

 $c_stat = array('All', $c_all),array('Today', $c_today), array('Week', $c_week), array('Month', $c_month),);

 $tables .= $div. "Reports count".build_table($c_stat, array('',''), "stat-table", array('',''), 'reportsgrid')."</div>";

 $tmp = SQLToArray("SELECT country, COUNT(*) AS CountRec,COUNT(*)/(SELECT COUNT(*) FROM reports)*100 AS percent FROM reports GROUP BY `country` ORDER BY CountRec DESC");

  for($i=0; $i<count($tmp); $i++)	{
 $tmp[$i]['country'] = "<img src='img/flags/".strtolower($tmp[$i]['country']).".png'> ".$tmp[$i]['country'];
               }
 $tables .= $div. "Country stats" .build_tables($tmp, array('Country', 'count', '%',"stat-table", array('','',''), 'reportsgrid')."</div>";							   	
